  ...
  const clearAllNotifications = async () => {
    try {
      ...
      try {
        ...
        // Fallback: Delete one by one if bulk clear route is not configured
        if (apiErr.response?.status === 404) {
          await Promise.allSettled(
            previousNotifications.map((n) => api.delete(`/notifications/${n.id}`))
          );
        } else {
          throw apiErr;
        }
      ...
      // Refresh to get actual server state if clear failed
      fetchNotifications();
    } finally {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 px-3 py-2 text-xs font-semibold text-slate-400 transition hover:bg-slate-800 hover:text-white"
            >
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-xs font-semibold text-emerald-400 transition hover:bg-emerald-500/15 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {markingAll ? (
            ...
            {notifications.length > 0 && (
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-rose-500/20 bg-rose-500/10 px-3 py-2 text-xs font-semibold text-rose-400 transition hover:bg-rose-500/15 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {clearingAll ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
        ...
        {!error && notifications.length === 0 && (
          <div className="mt-6 rounded-3xl border border-slate-800/80 bg-slate-900/40 px-6 py-16 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-slate-500">
              You don't have any notifications right now. New songs and
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button
                          ...
                          className="shrink-0 rounded-lg p-2 text-slate-600 opacity-0 transition hover:bg-rose-500/10 hover:text-rose-400 group-hover:opacity-100"
                        >
                          <Trash2 className="h-4 w-4" />
                        </button>
                      </div>
                      ...
                      <div className="mt-3 flex items-center justify-between gap-3">
                        ...
                        {!read && (
                          <button
                            ...
                            className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[10px] font-bold text-emerald-400 transition hover:bg-emerald-500/10"
                          >
